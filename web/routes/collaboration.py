...
async def list_comments(
    ...
    The caller must have access to the underlying notebook (either as
    owner or as an org member).  Without authorisation this endpoint
    previously let any authenticated user enumerate comments by guessing
    notebook / source / note IDs.
    """
    with get_conn() as conn:
        conditions = []
        params = []
...
async def create_comment(body: CommentCreate, user: dict = Depends(get_current_user)):
    ...
    When the comment is attached to a notebook (directly or via a source /
    note / parent comment), the caller must have access to that notebook.
    Replies inherit their target from the parent comment.  Untargeted
    personal comments are still allowed.
    """
    comment_id = str(uuid.uuid4())
    with get_conn() as conn:
        # Replies inherit notebook context from their parent.
        notebook_id = body.notebook_id
        if body.parent_id and not (notebook_id or body.source_id or body.note_id):
            parent = conn.execute(
                "SELECT notebook_id, source_id, note_id FROM comments WHERE id=?",
                (body.parent_id,),
            ).fetchone()
            if parent is None:
